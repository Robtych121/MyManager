...
def update_period_bal_from_function(id):
    # Incoming values
    total_wages = Transcation.objects.filter(period=id, type=1).values_list('value').aggregate(Sum('value'))
    if total_wages["value__sum"] == None:
        total_wages["value__sum"] = 0
    total_extraincome = Transcation.objects.filter(period=id, type=3).values_list('value').aggregate(Sum('value'))
    if total_extraincome["value__sum"] == None:
        total_extraincome["value__sum"] = 0
    ...
    # Outgoing values
    total_expenses = Transcation.objects.filter(period=id, type=2).values_list('value').aggregate(Sum('value'))
    if total_expenses["value__sum"] == None:
        total_expenses["value__sum"] = 0

    total_bills = Transcation.objects.filter(period=id, type=4).values_list('value').aggregate(Sum('value'))
    if total_bills["value__sum"] == None:
        total_bills["value__sum"] = 0
